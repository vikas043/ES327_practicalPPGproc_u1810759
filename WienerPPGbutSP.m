function PPGsigOut= WienerPPGbutSP()
clear all;
%%initialising output structure;
PPGsigOut=[];
...
%Load file containing PPG sensor readings,ECG and accelerometer data
raw=load('DATA_01_TYPE01.mat');

%%Load file containing HR ground truth in BPM
BPM_file=load('DATA_01_TYPE01_BPMtrace.mat');
BPM_truth=BPM_file.BPM0;
...
% 
% %Load file containing PPG sensor readings,ECG and accelerometer data
% raw=load('DATA_03_TYPE02.mat');
% 
% %Load file containing HR ground truth in BPM
% BPM_file=load('DATA_03_TYPE02_BPMtrace.mat');
% BPM_truth=BPM_file.BPM0;
%
%Uncomment to change the subject
% 
% %Load file containing PPG sensor readings,ECG and accelerometer data
% raw=load('DATA_05_TYPE02.mat');
% 
% %Load file containing HR ground truth in BPM
% BPM_file=load('DATA_05_TYPE02_BPMtrace.mat');
% BPM_truth=BPM_file.BPM0;

% Extracting data in to vectors
rawPPGc1=raw.sig(2,:);%PPG readings channel 1
rawPPGc2=raw.sig(3,:);%PPG readings channel 2
ACC_Ref_c3=raw.sig(4,:);%Accelerometer x-axis reading
ACC_Ref_c4=raw.sig(5,:);%Accelerometer y-axis reading
ACC_Ref_c5=raw.sig(6,:);%Accelerometer z-axis reading

%Standardized PPG averaged from both channels
rawPPGav1=0.5*(rawPPGc1-mean(rawPPGc1))/(std(rawPPGc1))+0.5*(rawPPGc2-mean(rawPPGc2))/(std(rawPPGc2));
%Mean PPG averaged from both channels
rawPPGav2=(0.5*rawPPGc1)+(0.5*rawPPGc2);

fs=125;%sampling frequency
t=0:1/fs:(length(rawPPGav1)/fs);%for plotting, time values from sample rate and index
t=t(2:length(t));
...
Wn=[fclow fchigh];
%Filter order(actual 8 as bandpass,2*n)
n=4;
...
%Passing the signal through butterworth filter
filPPGav1=filter(b,a,detPPGav1);
filPPGav2=filter(b,a,detPPGav2);

%Preprocessing accelerometer data for Adaptive stage
ACC_X = filter(b,a,ACC_Ref_c3);
ACC_Y = filter(b,a,ACC_Ref_c4);
ACC_Z = filter(b,a,ACC_Ref_c5);
...
[peaks,locs]=findpeaks(filPPGav1,'MinPeakHeight',0.1,'MinPeakDistance',30,'MinPeakProminence',0.03);
...
[peaks2,locs2]=findpeaks(filPPGav2,'MinPeakHeight',12,'MinPeakDistance',30,'MinPeakProminence',4);
%time at peak locations
locs_t2=locs2/fs;
%Plotting after second stage of preprocessing and bandpass filtering
%Plotting detected peaks
...
%Method 1 Early approcah,verification
%Standardized
%average time interval between peaks
avg_int_But_Stdz_met1=mean(diff(locs_t));
%BPM estimate from average interval
avg_BPM_But_Stdz_met1=(1/avg_int_But_Stdz_met1)*60;
%error from average HR ground truth
err_avgBPM_But_Stdz_met1=abs(avg_BPM_But_Stdz_met1-mean(BPM_truth));
%Unstandardized
%average time interval between peaks
avg_int_But_met1=mean(diff(locs_t2));
%BPM estimate from average interval
avg_BPM_But_met1=(1/avg_int_But_met1)*60;
%error from average HR ground truth
err_avgBPM_But_met1=abs(avg_BPM_But_met1-mean(BPM_truth));
...
if beat_det>1
    ...
    %error from average HR ground truth
    err_avgBPM_But_stdz=abs(avg_BPM_But_stdz-mean(BPM_truth));
end
...
if beat_det>1
    ...
    err_avgBPM_But=abs(avg_BPM_But-mean(BPM_truth));
end

%writing to output
PPGsigOut.avg_BPM_But_stdz=avg_BPM_But_stdz;
PPGsigOut.err_avgBPM_But_stdz=err_avgBPM_But_stdz;
PPGsigOut.avg_BPM_But=avg_BPM_But;
PPGsigOut.err_avgBPM_But=err_avgBPM_But;

%% Part 5 Adaptive Filtering %%
% Overall parameters
FFTres = 1000;   % FFT resolution - number of bins used
...
%Normalized filtering for HR estimation
 ...
 %averaged input power spectrum using upto 16 windows
        ...
        %Peak detection
        [peaks3,locs3]=findpeaks(PPG_ave_IFFT_Cln(i,:),'MinPeakHeight',0.15,'MinPeakProminence',0.1);
        [peaks4,locs4]=findpeaks(PPG_ave2_IFFT_Cln(i,:),'MinPeakHeight',12,'MinPeakProminence',4);
        
        
        for k=1:length(locs3)-1
            if locs3(k)<(locs3(k+1)-30)    
...
[peaks6,locs6]=findpeaks(PPG_Clean ,'MinPeakHeight',12,'MinPeakProminence',4);
...
%calculating BPM errors in the final signal normalized
        ...
        err_avgBPM_SigNorm=abs(mean(BPM_est_norm)-mean(BPM_truth));
        end
        
%calculating BPM errors in the final signal       
        for k=1:length(locs6)-1
            if locs6(k)<(locs6(k+1)-30)    
        ...
        err_avgBPM_Sig=abs(mean(BPM_est)-mean(BPM_truth));
        end
...
PPGsigOut.err_avgBPM_SigNorm=err_avgBPM_SigNorm;
PPGsigOut.err_avgBPM_Sig=err_avgBPM_Sig;
...
plot(BPM_est_norm);
hold on;
plot(BPM_truth,'ro','MarkerFaceColor','red','MarkerSize',3);
grid on;
title({'BPM (norm. sig) comparison to ground truth','Subject 1'})
xlabel('Window number');
ylabel('BPM estimate');
legend('Estimated BPM','BPM ground truth');

%Plotting estimated BPM against HR ground truth
figure();
plot(BPM_est);
hold on;
plot(BPM_truth,'ro','MarkerFaceColor','red','MarkerSize',3);
grid on;
title({'BPM comparison to ground truth','Subject 1'})
xlabel('Window number');
ylabel('BPM estimate');
legend('Estimated BPM','BPM ground truth');

%% Part 6-Plotting signals within a given interval %%
%introduce in earlier plots
%xlim([120 150]);
